...
class COMAP2PNG:
    def __init__(self, from_commandline=True, filename="", feeds=range(1,20), sidebands=range(1,4), frequencies=range(1,65), maptype="map", outname="outfile", outpath="", plottype="png", colorbarlims="[None, None]", jackknife="None"):
        ...
        if from_commandline:
            ...
            parser.add_argument("-j", "--jackknife", type=str, default="None0", help="Name of jackknife to plot.")
            ...
            self.jackknife  = args.jackknife[:-1]
            self.jackknife_nr = int(args.jackknife[-1])
            ...
            self.jackknife   = jackknife[:-1]
            self.jackknife_nr = int(jackknife[-1])
            if len(filename) < 0:
    ...
    def read_h5(self):
        ...
        if self.jackknife == "None":
            ...
            self.map_full = h5file["jackknives/map_" + self.jackknife][self.jackknife_nr][self.indexing]
            self.rms_full = h5file["jackknives/rms_" + self.jackknife][self.jackknife_nr][self.indexing]
            self.hit_full = h5file["jackknives/nhit_" + self.jackknife][self.jackknife_nr][self.indexing]
            print(np.max(self.map_full), self.jackknife, self.jackknife_nr)
    ...
    def make_maps(self):
        ...
        if USE_CTYPES:  # Ctypes implementation (much faster).
            ...
            maplib = ctypes.cdll.LoadLibrary("/mn/stornext/d16/cmbco/comap/jonas/comap_aux/map2png/maplib.so.1")  # Load shared library
            ...
            maplib.makemaps(map_full, rms_full, hit_full, self.map_out, self.rms_out, self.hit_out, nfeed, nband, nfreq, nx, ny)
    ...
    def make_title(self):
        ...
        if self.jackknife != "None":
            title += "jn: " + self.jackknife + "_" + str(self.jackknife_nr) + " | "
