...
# 1. Transfering yesterdays level1 files from OVRO to a temp directory. (Also, don't redirect stderr to mail, as rsync often fails).
...
$SCRIPT_DIR/transfer_level1_for_day.sh $YESTERDAY_MONTH $YESTERDAY_DAY >> $LOG_DIR/cronlog_daily_transfers.log 2>&1;\
