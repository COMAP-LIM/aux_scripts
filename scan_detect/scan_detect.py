...
def find_scan_ranges(mjd, status, scan_mode, feat, feat_mjd, field, params):
    ...
    for i in range(n_scans):
        index = n_scans - i - 1
        if (ends[index] - starts[index] < params['MINIMUM_SCAN_LENGTH'] / (24 * 60)):
            ends.pop(index)
            starts.pop(index)
            ...
            try:
                with h5py.File(file, mode="r") as fd:
                    ...
                    try:
                        ...
                        obs_id = -1
                        bad.append(file)
                    is_in_range = params['LATEST_OBSID'] >= obs_id >= params['EARLIEST_OBSID']
                    ...
                    elif new_format and is_in_range and not is_blacklisted:
                        # print('File: '+file)
...
require_tsys = params['REQUIRE_TSYS']
verb = params['VERBOSE_PRINT']
remove_eng = params['REMOVE_ENGINEERING_RUNS']
