    ...
    try:
        with h5py.File(filepath, mode="r") as my_file:
            ...
            try: 
                ...
                assert(sd_ind.shape[:4] == (3, n_det_ind, n_sb, 4))
            except:
...
def make_accept_list(params, accept_params, scan_data):
    ...
    # accept_list[:, 7, :] = False    
    n_sw = 14
    acc = np.zeros(len(stats_list) + 1 + n_sw)
    ...
    for i in range(n_mean):
        j = n_sw - i - 1
        sw_string = 'sw_%02i' % (j + 1)
        print(sw_string)
        stats = extract_data_from_array(scan_data, sw_string)
        div.append(stats)
    div = np.nanmean(np.array(div), 0)
    for i in range(n_sw):
        sw_string = 'sw_%02i' % (i + 1) 
        stats = extract_data_from_array(scan_data, sw_string) / div
        cuts = accept_params.sw_cuts[sw_string]
        accept_list[np.where(stats < cuts[0])] = False
        accept_list[np.where(stats > cuts[1])] = False
        acc[len(stats_list) + i + 1] = np.nansum(acceptrate[accept_list]) / (n_scans * 19 * 4)
        print(acc[len(stats_list) + i + 1], sw_string, cuts)
